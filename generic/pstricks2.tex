...
%
\def\fileversion{2.00a}
\def\filedate{2005/02/17}% RN; was: \def\filedate{2004/11/23}
%%% begin RN 2005-02-12 
...
\input pstricks.con
%%% begin RN 2005-01-28
\let\pstheader@sav\pstheader
\let\pstverb@sav\pstverb
\let\pstVerb@sav\pstVerb
\def\PSTricksOn{%
\let\pstheader\pstheader@sav
\let\pstverb\pstverb@sav
\let\pstVerb\pstVerb@sav
\PSTrickstrue}
%%% end
\def\PSTricksOff{%
...
{\ifPSTricks\else\PSTricksOff\fi}
%%% end
...
\def\pst@getcoors#1#2{%
  ...
  \def\pst@coors{}%
  \pst@@getcoors}
\def\pst@@getcoors(#1){%	read a (..)(..)(..)... sequence
  \pst@@getcoor{#1}%
...
% 
%   there is no real need _not_ to choose \SpecialCoor as default  (hv 20050515)
%\NormalCoor
%
...
%
% We noe have the hardware power to choose \SpecialCoor as default (hv 20050514)
\SpecialCoor
%
\begingroup
\catcode`\|=13
\catcode`\;=13
\catcode`\!=13
\gdef\pst@activecoor{%		define the shortcuts for special coordinates
  \def|{\string|}%		| double
  \def;{\string;}%		; polar
  \def!{\string!}}%		! PS
\endgroup
...
\define@key[psset]{pstricks}{swapaxes}[true]{%
%%% end
  \@nameuse{@pst#1}%
  \if@pst
...
\newif\ifshowpoints
%%% begin RN 2005-02-12
\define@key[psset]{pstricks}{showpoints}[true]{\@nameuse{showpoints#1}}
...
\define@key[psset]{pstricks}{doubleline}[true]{%
%%% end
...
\define@key[psset]{pstricks}{shadow}[true]{%
%%% end
  \@nameuse{psshadow#1}%
  \pst@setrepeatarrowsflag}
...
\define@key[psset]{pstricks}{dashadjust}[true]{\@nameuse{psdashadjust#1}}
%%% end
\def\psls@dashed{%
  \ifpsdashadjust
...
\define@key[psset]{pstricks}{hatchstyle}{%
  \@ifundefined{pshs@#1}%
...
\def\psk@arrowA{}
\def\psk@arrowB{}
\edef\pst@arrowtable{,<->,<<->>,>-<,>>-<<,(-),[-],)-(,]-[,|>-<|,H-H,h-h,|<*-*>|,|<->|}% hv
\begingroup
  \catcode`\<=13
  \catcode`\>=13
  \catcode`\|=13
...
\define@key[psset]{pstricks}{ArrowFill}[true]{\@nameuse{psArrowFill#1}}
%%% end
...
\def\tx@MRestore{% fixed a typo in the original pstricks
...
\define@key[psset]{pstricks}{boxsep}[true]{\@nameuse{psboxsep#1}}
%%% end
\def\pst@useboxpar{%
\use@par
\if@star
...
%
% hv 2005-05-05 ---> begin
% to make arc the other way round working
\def\psarc@v{%
  x y r
  angleA
...
\define@key[psset]{pstricks}{frame}[true]{\@nameuse{Pst@frame#1}}
%%%\newpsstyle{psframestyle}{linewidth=0.1pt,linestyle=dashed}
\newpsstyle{psframestyle}{linewidth=0.4pt,linestyle=dashed,framesep=0pt}
%%% end
%%% begin RN 2005-02-06
\let\pst@llx\pst@dima
\let\pst@lly\pst@dimb
\let\pst@urx\pst@dimc
\let\pst@ury\pst@dimd
\def\psk@addto@llx#1{\psk@addto@bb{llx}{#1}}
\def\psk@addto@lly#1{\psk@addto@bb{lly}{#1}}
\def\psk@addto@urx#1{\psk@addto@bb{urx}{#1}}
\def\psk@addto@ury#1{\psk@addto@bb{ury}{#1}}
\let\psk@@addto@llx\relax
\let\psk@@addto@lly\relax
\let\psk@@addto@urx\relax
\let\psk@@addto@ury\relax
\def\psk@addto@bb#1#2{%
  \begingroup
    \pst@dimg=#2\relax
    \@nameuse{pst@#1}=\z@\@nameuse{psk@@addto@#1}%
    \advance\pst@dimg\@nameuse{pst@#1}%
    \edef\x{\def\@nameuse{psk@@addto@#1}{%
      \advance\@nameuse{pst@#1}\the\pst@dimg}}%
  \expandafter\endgroup\x
}
%%% end
%%% begin RN 2005-02-03
\define@key[psset]{pstricks}{trim}{%
  \pst@expandafter\pst@@trim#1 %
    {\pst@missing} {\pst@missing} {\pst@missing} {\pst@missing}\@nil
  \psk@addto@llx{\pst@dima}\psk@addto@lly{\pst@dimb}%
  \psk@addto@urx{\pst@dimc}\psk@addto@ury{\pst@dimd}%
}
\def\pst@@trim#1 #2 #3 #4 #5\@nil{%
  \pssetlength\pst@dima{#1}\pssetlength\pst@dimb{#2}%
  \pssetlength\pst@dimc{#3}\pssetlength\pst@dimd{#4}%
}
%%% end
...
%{Type \space <return> \space to procede.}%
  \fi%
%%% begin RN 2005-02-06
  \psk@@addto@llx\psk@@addto@lly
  \psk@@addto@urx\psk@@addto@ury
%%% end
  \ht\pst@hbox=\pst@dimd%
  \dp\pst@hbox=-\pst@dimb%
  \setbox\pst@hbox=\hbox{%
...
\def\resetPSTOptions{% hv
  \psset[pstricks]{%
       ...
       arrows=-,
       liftpen=0,
       linetype=2,% otherwise there maybe a problem when dashed lines
       gangle=0,
       curvature=1 .1 0,
       dotsize=2pt 2,
       dotangle=0,
